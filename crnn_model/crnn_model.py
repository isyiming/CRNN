...
class ShadowNet(cnn_basenet.CNNBaseModel):
    ...
    def __feature_sequence_extraction(self, inputdata):
        ...
        :return:
        这里我真的不想多注释，无非是卷积池化激活，单纯在代码里堆砌没有任何说明的价值
        不过77行 83行 85行的if else语句 作用：是判断你是要train还是test ，如果是test，直接跳过前面的两个if了
        网络是如何起作用的，还是看论文吧
        """
        conv1 = self.__conv_stage(inputdata=inputdata, out_dims=64, name='conv1')  # batch*16*50*64 这个函数是整合了一次卷积和一次池化操作，卷积后，线性激活，然后选取邻域最大特征池化
        conv2 = self.__conv_stage(inputdata=conv1, out_dims=128, name='conv2')  # batch*8*25*128 同上
        conv3 = self.conv2d(inputdata=conv2, out_channel=256, kernel_size=3, stride=1, use_bias=False, name='conv3')  # batch*8*25*256 单纯卷积操作
        relu3 = self.relu(conv3) # batch*8*25*256  Relu线性激活
